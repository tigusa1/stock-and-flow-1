...
from simulation_v2 import run_simulation
...
def run_streamlit():
    ...
    if is_debugging():
        ...
        with col1:
            with st.expander("Basic Settings", expanded=True):
                ...
                # n_months = st.slider("Simulation duration (months)", 2, 4, 2, help=".")
                ...
                n_months = st.slider("Simulation duration (months)", 24, 84, n_months_0, 3,
                                        help=".")
        ...
        with col1:
            ...
            with st.expander("Delays in NOA", expanded=True):
                ...
                T1_reduction_in_burn_rate = st.slider("Time of reduced spending (months)",
                                                      0.0, 12.0, T1_reduction_in_burn_rate_0, 0.5) + \
                    reimbursement_duration
        ...
        P_duration = 52
        n_projects = round(n_projects_yr / 12 * (n_months - P_start*12/52 - P_duration*12/52))
    ...
    (cash_history, non_reimb1, non_reimb2, idc_log, idc_2_log, inst_paid_log, total_avg_reimbursement,
     ...
     proj_types) = run_simulation(  # v2 return np grants arrays
        ...
        P_start = P_start, P_end = P_end, P_duration = P_duration
    )
    ...
    for t in range(T-1):
        if t < d:
            ...
            cash_balance[t+1] = cash_balance[t]
        else:
            cash_balance[t+1] = cash_balance[t] - total_spend[t] + total_reimbursement[t]
        # print(f"{t:.0f}: {cash_balance[t]:.0f}, {total_spend[t]:.0f}, {total_reimbursement[t]:.0f}")
    ...
    if flag_original_plot:
        ...
        axs[0].plot(months, sm(cash_balance[d:]), label="Cash Balance", linewidth=2)
        ...
        axs[3].plot(months, np.cumsum(np.array(idc_log[d:]) - np.array(idc_2_log[d:])), label="Cumulative loss of IDC",
                    linewidth=2)
    ...
    # Generate data
    t = np.arange(T)
    # burns,burns_BL = generate_projects(t, n_grants, decline_month, decline_factor, T,
    ...
    png_path = make_project_activity_animation(t, reimbursement_types, spend_types, T, time_marker, animate=False,
                                               reimbursement_duration=d)  # DEBUG
    # png_path = make_project_activity_animation(t, burns, burns_BL, T, time_marker, animate=False,
    #                                            reimbursement_duration=d)  # DEBUG
    # png_path = make_project_activity_animation(t, reimbursement_by_project, burns, T, time_marker,
    ...
    if is_debugging():
        ...
        ax.axis('off')
        plt.show()
        # gif_path = make_project_activity_animation(t, burns, burns_BL, T, decline_month) # DEBUG
    else:
        with col3:
            ...
            with st.spinner("Creating animation... please wait ⏳"):
                gif_path = make_project_activity_animation(t, burns_types, burns_types, T, time_marker, animate=False,
                                                           reimbursement_duration=d,
                                                           del_T_p_non_reimb=del_T_p_non_reimb)
                # gif_path = make_project_activity_animation(t, burns, burns_BL, T, time_marker,
