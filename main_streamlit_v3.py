...
def render_ui():
    """
    Build all Streamlit UI and return a dict of all parameters needed
    by the simulation and plotting routine.
    """
    ...
    if is_debugging():
        ...
        col3 = None  # not used in debugging mode
    else:
        ...
        with col1:
            with st.expander("Basic Settings", expanded=True):
                cash_init = st.slider(
                    "Initial cash balance ($MM)", 0, 8000, cash_init_0, step=100
                )
                cash_init *= 1000
                # For now, use default number of projects/year and award size:
        ...
        with col2:
            ...
            with st.expander("Reimbursement Delays", expanded=True):
                reimbursement_duration = st.slider(
                    ...
                    1,
                    10,
                    reimbursement_duration_0,
                )
                future_reimbursement_duration = st.slider(
                    "Future delay (weeks)",
                    1,
                    10,
                    future_reimbursement_duration_0,
                )
    ...
    # Return all parameters needed by the simulation
    return dict(
        ...
        cash_init=cash_init,
        n_projects_yr_0=n_projects_yr_0,
        max_award=max_award,
...
def run_simulation_and_plot(params):
    ...
    if flag_original_plot:
        ...
        axs[0].plot(
            non_reimb1,
            label="Receivable (Reimbursable Projects)",
            linestyle="--",
        )
        axs[0].plot(
            non_reimb2,
            label="Receivable (Non-reimbursable Projects)",
            linestyle=":",
        )
    else:
...
# Use session_state to persist "run" flag across reruns
...
if st.session_state["run_sim"]:
    run_simulation_and_plot(params)        plot_individual_projects = False
    else:
        flag_use_saved = False
        # flag_use_saved = st.toggle("Use saved values")
